  ...
  const translations = {
    uk: {
      ...
      faq_a2: [
        ...
        '🗄️ Унікальний префікс таблиць WordPress',
      ],
    ...
    en: {
      ...
      faq_a2: [
        ...
        '🗄️ Unique WordPress table prefix',
      ],
...
`;
  ...
  // НОВА ФУНКЦІЯ: генерація унікального префікса таблиць
  function generateTablePrefix() {
    const chars = 'abcdefghijklmnopqrstuvwxyz0123456789';
    let prefix = '';
    for (let i = 0; i < 6; i++) {
      prefix += getRandomChar(chars);
    }
    return prefix;
  }
  ...
  function buildProjectData() {
    ...
    // НОВЕ: генеруємо унікальний префікс таблиць
    const tablePrefix = generateTablePrefix();
...
${translations.en.sql_comment_important}`;
  ...
  // ОНОВЛЕНО: додано унікальний префікс таблиць
  function updateWpConfigSummary(dbName, dbUser, dbPass, tablePrefix) {
 ...
 * Use a custom, non-default prefix: (e.g.'${tablePrefix}_') instead of 'wp_'.
 * 
 * For EXISTING sites:
 * Do not change the prefix unless you know exactly what you're doing,
 * as it requires a full database migration and may break the site. 
 */
\$table_prefix = '${tablePrefix}_';`;
  ...
  function generateAll() {
    ...
    updateWpConfigSummary(
      ...
      projectData.dbUser,
      projectData.dbPass,
      projectData.tablePrefix, // НОВЕ: передаємо префікс таблиць
    );
...
========================================`;
  ...
  function refreshDbPassword() {
    ...
    updateWpConfigSummary(
      ...
      currentDbPass,
      'wp', // тимчасовий префікс, він не змінюється при оновленні пароля
    );
